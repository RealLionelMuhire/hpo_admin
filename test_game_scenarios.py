...
def create_test_players():
    ...
    print(f"   Using test players: {', '.join([p['name'] + ' (ID: ' + str(p['id']) + ')' for p in test_players])}")
    return test_players
...
def test_two_player_game():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"     Team 1: {', '.join([f'{p['player_name']} (ID: {p['player_id']})' for p in team1_players])}")
            print(f"     Team 2: {', '.join([f'{p['player_name']} (ID: {p['player_id']})' for p in team2_players])}")
